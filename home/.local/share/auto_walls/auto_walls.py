...
class AutoWalls(State):
    ...
    def set_wallpaper(self, wallpaper: str, do_change_index=True):
        ...
        lock_file = os.path.expanduser(
            '~/.local/state/auto_walls/auto_walls.lock')

        # Standard lock check to prevent overlap
        ...
        try:
            ...
            # ONLY CALL MATUGEN
            # Everything else (awww, waybar, swaync) is in config.toml
            subprocess.run(["matugen", "image", wallpaper,
